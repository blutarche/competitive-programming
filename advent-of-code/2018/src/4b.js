...
const main = async () => {
  ...
  const mostSleepGuard = _.chain(logs)
    .reduce(
      (result, log) => {
        let { guardLogs, currentGuard } = result
        if (log.guardId) {
    ...
    .map((guardLog, id) => {
      const s = _.reduce(
        guardLog.sleepLogs,
        (result, sleepLog, minute) => {
          let { count, mostSleepMinute, maxSleep } = result
          count += sleepLog
          if (count > maxSleep) {
            maxSleep = count
            mostSleepMinute = minute
          }
          return { count, mostSleepMinute, maxSleep }
        },
        { count: 0, mostSleepMinute: null, maxSleep: -1 }
      )
      console.log(s)
      return {
        id,
        mostSleepMinute: s.mostSleepMinute,
        maxSleep: s.maxSleep
      }
    })
    .maxBy(log => log.maxSleep)
    .value()

  const answer = mostSleepGuard.id * mostSleepGuard.mostSleepMinute
  console.log(mostSleepGuard)
  console.log(answer)
